...
export default function TambahListToDoScreen({ navigation, route }){
    ...
    const [date2, setDate2] = useState("Deadline");
    ...
    const addLTD = () => {
        ...
        if(nama === "" || bgColor === "" || txtColor === "" || ndate === "" || tgjwb === "" || notif === ""){
            ...
            lib.newLTD(krj, nama, ndate, tgjwb, email, bgColor, txtColor, notif).then((json) => {
    ...
    useEffect(() => {
        ...
        responseListener.current = Notifications.addNotificationResponseReceivedListener(response => {
          lib.navigate(response, "TambahListToDo", "");
        });
    ...
    return (
        <NativeBaseProvider>
            <Header
                ...
                header={
                    <Box>
                        ...
                        <Text fontWeight={'bold'} fontSize={20}>Tambah Kegiatan</Text>
                    </Box>
                }
            ...
            />
            <ScrollView>
                <VStack style={err !== -2 && err !== -3 && err !== -4 ? styles.d_flex : styles.d_none} px='30px' py='30px'>
                    <Box>
                        <Input
                            InputLeftElement={
                                <Icon
                    ...
                    <Box bg='rgba(28, 163, 255, 0.3)' mt={3} rounded={'sm'} h={'50px'} style={{display: 'flex', justifyContent: 'center'}}>
                        <HStack>
                    ...
                    <Box bg='rgba(28, 163, 255, 0.3)' rounded='sm' h='50px' mt={3} style={{display: 'flex', flexDirection: 'row', alignItems: 'center'}} pl={3} pr={35}>
                        <Select dropdownIcon={
                        <Icon
                            as={down_icon}
                            size={8}
                            color='#1CA3FF'
                        />
                        } placeholder='Penanggung Jawab' placeholderTextColor={'#1CA3FF'} borderWidth={0} minWidth="100%" onValueChange={(val) => setTgjwb(val)} fontSize={"md"} fontWeight={"normal"}>
                            {
                                lusr.map((data) => {
                    ...
                    <Box bg='rgba(28, 163, 255, 0.3)' rounded='sm' h='50px' mt={3} style={{display: 'flex', flexDirection: 'row', alignItems: 'center'}} pl={3} pr={35}>
                        ...
                        <Icon
                            ...
                            color='#1CA3FF'
                        />
                        } placeholder='Munculkan Notifikasi' placeholderTextColor={'#1CA3FF'} borderWidth={0} minWidth="100%" onValueChange={(val) => setNotif(val)} fontSize={"md"} fontWeight={"normal"}>
                            {
                                lntf.map((data) => {
                    ...
                    <Box w='100%' mt={30} style={{display: 'flex', alignItems: 'center'}}>
                        <Button w='50%' style={styles.tsk_btn_main} size="lg" p={5} isLoading={isSave} onPress={addLTD}>
                            <Text style={{fontSize: 16, fontWeight: 'bold', color: '#FFFFFF'}}>Tambah</Text>
                        </Button>
                    </Box>
                </VStack>
                ...
                <VStack style={err === -4 ? styles.d_flex : styles.d_none} space={8} py={4} px={4}>
                    <Center>
                        {
                            lib.upgrade_image()
                        }
